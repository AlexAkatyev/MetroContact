...
//           Qt.Window
    ...
    function getGaugeSize(){
        var h = itGauge.height - gaugeVertical.anchors.topMargin - txVertical.font.pixelSize * 2 - itPicket.height;
        var w = itGauge.width / 2;
        return h > w ? w : h;
    }
    ...
    StackLayout {
        ...
        Item {
            ...
            ImpGauge{
                ...
                anchors.top: parent.top
                anchors.topMargin: gaugeVertical.anchors.topMargin
                anchors.right: parent.right
                width: getGaugeSize()
                height: getGaugeSize()
            }
...
Программа отслеживает, чтобы был запущен только один экземпляр программы MetroContact или
программы IMP. Запуск одновременно нескольких экземпляров программы или одновременно с
программой IMP не возможен!
...
Если программа неожиданно завершит свою работу, без сохранения журнала по пункту 4,
то возможно восстановление части измерений, т.к. измерения временно дублируются
в файле cachMetroContact.csv рабочего каталога программы.
"
                }
            }
        } // Item itManual
