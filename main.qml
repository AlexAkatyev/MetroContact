...
Window {
    ...
    visible: true
    title: "Катучий шаблон М-009  v1.2.6"
...
"
    ...
    необходимо открыть вкладку НАСТРОЙКА, указать стартовый пикет и расстояние от стартового пикета,
    откуда начнется измерение.
    ...
    После нажатия кнопки ПАУЗА возможно перемещение тележки в другую измерительную позицию,
    предполагающую изменение расстояния от пикета. В этот момент можно внести новое значение
    расстояния от предыдущего пикета и далее нажать кнопку ПРОДОЛЖИТЬ.
    ...
    Если мигание отстуствует, это говорит о том, что данные с соответствующего датчика
    ...
    измерительное оборудование в соответствие с разделом 5.
    ...
    5. Проблемы взаимодествия с оборудованием
    Если в процессе измерения возникают проблемы с оборудованием, и требуется оборудование выключить
    и включить, программу перезапускать не требуется.
    Для этого:
        1) Остановить измерения, нажав на кнопку ПАУЗА.
        2) Отсоединить оборудование от компьютера.
        3) Во вкладке НАСТРОЙКА нажать на синюю кнопку подключения оборудования, возле которой
        указан порт, через который было подключение. И дождаться сообщения \"Нет соединения\".
        4) Подсоединить оборудование к компьютеру.
        5) Снова нажать на синюю кнопку подключения оборудования и дождаться, когда появится сообщение
        с указанием порта подключения.
        6) Продолжить измерения, нажав на кнопку ПРОДОЛЖИТЬ.
